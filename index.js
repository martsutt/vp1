...
app.get("/thumbgallery", (req, res) => {
  let notice = "";
  const sqlreq =
    "SELECT file_name, orig_name, alt_text FROM photos WHERE privacy = ?";
  const privacyValue = 3;
  conn.query(sqlreq, [privacyValue], (err, results) => {
    if (err) {
      console.error(err);
      notice = "Pöidlapiltide galerii laadimine ebaõnnestus";
      res.render("thumbgallery", { photos: [], notice: notice });
    }
    notice = "Pöidlapiltide galerii laadimine õnnestus";
    res.render("thumbgallery", { photos: results, notice: notice });
  });
});
